...
- At the cap, spend the tick on the pipeline instead: the full sweep of [[feedback-watch-pr-comments]] — address feedback, fix red CI, merge what's approved ([[feedback-merge-on-approval]]), run auto-lane merges ([[feedback-automerge-risk-lanes]]), transition merged tickets ([[feedback-ticket-done-on-merge]]). Merges and closes free slots naturally.
...
- The cap works in BOTH directions: **below the cap, a non-blocked ready task is ALWAYS pulled immediately** — PRs sitting in review are never a reason to idle. The loop keeps drawing from the ready queue continuously ([[feedback-queue-triage-plan]]); it never waits for its earlier PRs to merge unless a `depends-on` edge requires it.
- Since every PR is born a draft ([[feedback-status-mirrors-work]]), draft-vs-ready does NOT decide the count. The cap counts every open registered PR **except** those parked by [[feedback-fail-gracefully-timebox]] (marked `blocked` in the registry): gates-phase drafts are active work heading to review and count; blocked parks sit in the human triage lane, cannot merge, and counting them would let three stuck tasks freeze the queue forever.
