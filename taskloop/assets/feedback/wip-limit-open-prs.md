...
- Draft PRs parked by [[feedback-fail-gracefully-timebox]] do **NOT** count toward the cap: their tickets already sit in Blocked, in the human triage lane, and they cannot merge — counting them would let three stuck tasks freeze the queue forever, defeating "one stuck issue must not stall the queue". The cap counts PRs that consume review bandwidth: open, non-draft, loop-owned.
