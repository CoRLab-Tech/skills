...
- **Under `LOOP_AUTOMERGE=on`:** a fully-gated, registered PR whose lane reads `auto` in both the registry and a fresh read of the ticket's marker, with ZERO human touch, merges through the auto lane per [[feedback-automerge-risk-lanes]] — after approved-PR merges, never before (an approval is a human waiting).
...
- Priority order per tick: red CI on an open PR ([[feedback-regression-is-blocker]]) > requested changes > unanswered questions > approved-PR merges > auto-lane merges ([[feedback-automerge-risk-lanes]]) > merged-PR ticket transitions > new ready task.
