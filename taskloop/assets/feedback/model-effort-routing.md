...
- **The marker contract (mechanically enforced):** every agent prompt the loop writes BEGINS with `[LOOP-AGENT issue:<key> tier:<light|standard|deep> verdict:<yes|no>]`. A PreToolUse hook installed at init (`hook-agent-routing.mjs` in the memory dir) reads the marker on every spawn and **upgrades under-modeled calls in place**: verdict/deep on anything below the top tier → pinned to the top model; standard on the fastest model → raised to balanced. It never downgrades, never blocks, and ignores prompts without the marker — prose can be forgotten, the hook cannot.
