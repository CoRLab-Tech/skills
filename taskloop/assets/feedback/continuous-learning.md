...
- **The promotion bar — three tests, all of them.** A candidate becomes a durable rule only if it (1) is **verified**, not merely plausible: it was observed to be true, or the correction came from the owner; (2) **names the failure pattern** it prevents, concretely enough that a future agent recognizes the situation ("a subagent that backgrounds a gate and ends its turn" beats "be careful with gates"); and (3) **records the alternative that was ruled out**, and why — the road not taken is what stops the next agent from re-litigating the decision. A candidate that fails any of the three is a note, not a rule: leave it in the PR or ticket comment where it belongs.

  **Why the bar exists:** the rule set is read every tick, so every weak rule is a permanent tax on every future iteration — context spent, attention diluted, and a real rule harder to find among the noise. A rule set that grows without a bar converges on being ignored. Prefer strengthening an existing rule with new evidence over adding a sibling; prefer deleting a rule that stopped being true over leaving it to rot.
...
- Lessons apply from the **very next iteration** — don't wait for a restart. The QA agent ([[feedback-pr-gate-passes]]) receives the accumulated lessons as part of the project conventions it checks against.
