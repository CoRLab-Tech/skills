...
2. ONLY when ALL gates are green does the loop, in one step: `gh pr ready <N>` → register the PR for the watcher → **post the PR link as a comment on the tracker issue** → move the issue to the review status ([[feedback-status-mirrors-work]]) → ping the owner once ([[feedback-telegram-notify-owner]]). These are not five optional niceties; they are one event — the "gates passed" signal. Missing any of them means the step is not done.
