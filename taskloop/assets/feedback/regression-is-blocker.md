...
- **Owner cost opt-out (explicit directive only):** when the owner directs it — e.g. branch-push bursts throttle the CI runner (symptom: seconds-fast failures with no logs) or Actions minutes cost real money — the loop may treat its LOCAL foreground full-suite gates as authoritative for PR branches instead of watching branch CI, and must not re-run throttled remote jobs. Two things survive the opt-out untouched: a GENUINE locally-reported failure is still a hard blocker, and the post-merge `main` backstop above becomes non-negotiable (it is then the only merge-result verification left). Under the opt-out, every "green CI" precondition in other rules ([[feedback-merge-on-approval]]'s merge check, the step-11 post-flip watch) reads as "local full-suite gates green" — an unwatched branch CI is neither green nor red, and must not deadlock the pipeline. Local caches can also mask a missing/undeclared dependency that only fails on clean install — if `main` goes red on install/lockfile while local was green, trust `main` and fix the manifest.
