...
- **Reconcile at start:** for tickets the loop itself put In Progress (they're in its registry / carry its transition trail), if the loop restarts and finds no active work and no open PR for them → move to Blocked with a note. Never touch tickets a human moved.
