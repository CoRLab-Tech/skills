...
4. **Model routing profile** — ask via `AskUserQuestion` ("Which model routing profile for this project's agents?"):
   - **High (Recommended, default)** — top = `opus` for verdicts + deep-tier work; `sonnet` balanced; `haiku` light. Strong quality at standard cost.
   - **Mythos (extra)** — top = `fable` (the Mythos-class tier above opus) for verdicts + deep; `sonnet` balanced; `haiku` light. Maximum judgment quality, extra cost.
   - **Custom** — follow-up questions: top model (`fable`/`opus`), top scope (`verdict` only / `verdict+deep` / `all` tiers), balanced model (`sonnet`/`opus`).

   Record the answers for the hook render below and write `$MEM/feedback/model-routing-profile.md` (standard feedback frontmatter, name `feedback-model-routing-profile`) stating: the chosen top model + scope + balanced model, the date, and the note that the tiers and the verdict-pin principle come from the universal `model-effort-routing` rule — only the model ids are per-project. Index it in `MEMORY.md`.
...
Render `assets/hook-agent-routing.mjs.tmpl` from the routing profile chosen in Step 4: `{{TOP_MODEL}}` → the profile's top (`fable` for Mythos, `opus` for High, custom's answer), `{{TOP_SCOPE}}` → `verdict+deep` for Mythos/High or custom's answer (`verdict` / `verdict+deep` / `all`), `{{BALANCED_MODEL}}` → `sonnet` (or custom's answer). Write to `$MEM/hook-agent-routing.mjs`.
